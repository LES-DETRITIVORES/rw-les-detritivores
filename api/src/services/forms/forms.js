...
export const emailForm = async ({ id }) => {
  ...
  const html =
    'Bonjour ' + form.firstname + ' !<br/><br/>' +
    'La coopérative inclusive LES DETRITIVORES vous confirme la bonne réception de votre demande. ' +
    "Nous ne manquerons pas d'y répondre au plus vite.<br/><br/>" +
    '<hr/>' +
    'Numéro de demande : ' + form.id + '<br/>' +
    (form.company ? 'Société : ' + form.company + '<br/>' : '') +
    'Contact : ' + form.firstname + ' ' + form.lastname + '<br/>' +
    'Adresse : ' + form.location + '<br/>' +
    'Mél : ' + form.email + '<br/>' +
    'Tél : ' + form.phone + '<br/>' +
    'Demande : ' + form.subject + '<br/>' +
  ...
  const email = await sendEmail({ to: form.email, bcc: ['bonjour@les-detritivores.co'], subject, text, html })
